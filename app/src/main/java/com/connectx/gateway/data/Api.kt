...
class Api(private val prefs: Prefs) {
    ...
    fun fetchAdminProfile(shopId: String? = null): AdminProfile {
        ...
        try {
            if (prefs.adminToken.isNotBlank()) {
                val r = call("device/auth/profile", token = prefs.adminToken)
                val p = AdminProfile(
    ...
    fun shops(): List<Shop> {
        val r = call("device/workspaces", token = prefs.adminToken)
     ...
     * (Gateways → Pair new gateway) binds this phone to a workspace and
    ...
    fun pairDevice(
        ...
        val conn = Connection(
            shopId = shop.getString("id"),
            shopName = shop.optStringOrNull("name") ?: "Workspace",
            shopAddress = shop.optStringOrNull("address") ?: "",
            adminId = "",
            adminEmail = "",
            adminName = "",
